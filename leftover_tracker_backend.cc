...
void SerializeLeftoverRecordToJSON(
  ...
  double quantity = record.GetQuantityOunces();;
...
LeftoverRecord DeserializeLeftoverRecordFromJSON(const rapidjson::Value &json_obj) {
  LeftoverRecord record;
     ...
     */
    //if (json_obj.HasMember("leftover_reason_") && json_obj["leftover_reason_"].IsString()) {
     ...
     */
    //if (json_obj.HasMember("disposal_mechanism_") && json_obj["disposal_mechanism_"].IsString()) {
...
LeftoverRecord QueryStringToLeftoverRecord(
  ...
  record.SetQuantityOunces(std::stod(query_string.get("qty_in_oz")));
  ...
  // using the setter in LeftoverRecord class.
  record.SetDisposalMechanism(query_string.get("disposal mechanism"));
